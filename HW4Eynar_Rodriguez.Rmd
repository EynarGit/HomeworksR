...
```{r}
ggplot(diamonds) + geom_histogram(aes(x=price),bins=40)
```
#2.2. Agregue una escala logarítmica de base 10 y devuelva el gráfico para ver la
#diferencia.

```{r}
ggplot(diamonds) + geom_histogram(aes(x=price),bins=40) + scale_x_log10()
```
...
#devuelva el gráfico.
```{r}
ggplot(diamonds)+ geom_boxplot(aes(x = cut, y = price )) + scale_y_log10()
```
...
```{r}
ggplot(filtered_price, aes(x=cut, y=depth)) + geom_col(aes(fill=clarity)) + facet_wrap(~ clarity)
```
...
```{r}
ggplot(cut_premium, aes(x=clarity, y=maxPrice)) + geom_col(aes (fill=clarity))
```
